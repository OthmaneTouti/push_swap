...
/*   Updated: 2023/12/01 20:22:45 by ottouti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
to b without checking anything (except if stack_a has a length of 4, in which
case it pushes only one node) so that stack_b has a minimum and maximum node.
This will allow us to find targets and calculate the cheapest node to move in
the init_nodes functions. The nodes are then pushed to stack_b and these
previous steps are repeated until there is only 3 nodes left in stack_a.
The program then performs a tiny_sort on stack_a to sort 3 integers, which is
a trivial operation. We then move all the nodes back to stack_a while finding
the right positions for them and we bring the minimum of stack_a on top.*/
